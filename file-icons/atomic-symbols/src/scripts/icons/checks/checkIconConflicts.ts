...
const checkForConflictsInFileIcons = (fileIconDefinitionType: 'fileExtensions' | 'fileNames') => {
  const icons: Record<string, string> = {};
  for (const icon of fileIcons.icons) {
    if (!icon[fileIconDefinitionType]) {
      continue;
    }
    for (const definition of (icon[fileIconDefinitionType] ?? []).map((d: string) => d.toLowerCase())) {
...
const checkFolderIcons = () => {
  for (const theme of folderIcons) {
    ...
    for (const icon of theme.icons) {
      for (const folderName of icon.folderNames.map((f: string) => f.toLowerCase())) {
...
const checkLanguageIcons = () => {
  ...
  for (const langIcon of languageIcons) {
    for (const id of langIcon.ids.map((id: string) => id.toLowerCase())) {
      if (icons[id]) {
...
const handleErrors = () => {
  if (
    ...
    console.log('> Atom Material Icons:', green('Passed icon conflict checks!'));
  }
};
...
const printErrorMessage = (icons: any, definitionType: string) => {
  ...
  for (const key of keys) {
    ...
    console.log(
      red(`For ${definitionType} "${key}" are ${conflictIcons.length} icons defined: [${conflictIcons.join(', ')}]`)
    );
  }
};
