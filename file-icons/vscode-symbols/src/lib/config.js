...
function getWorkspaceConfiguration() {
	...
	for (const key of configKeys) {
		...
		config[PKG_PROP_MAP[key]] =
			valueGroup.workspaceValue || valueGroup.globalValue || defaultState[PKG_PROP_MAP[key]];
	}
...
function updateConfig(config) {
	...
	const useDefaultAssociations = vscode.workspace
		.getConfiguration("symbols")
		.get("defaultAssociations", true);
